...
describe('voucher_codes', function(){
    it('should generate code of request length',
       function(){
        var length = 5; var code = voucher_codes.generate
            ({length: length})[0];expect
            (code.length).toBe(length);
       }
      );
    it('should generate code of default length',
       function(){ var default_length = 8;
        var code = voucher_codes.generate({})[0];
        expect(code.length).toBe(default_length); 
                 }
      );
    it('should generate code if no config provided',
       function(){ var default_length = 8;
        var code = voucher_codes.generate()[0];expect(code.length).toBe
            (default_length);
                 }
      );
    ...
    it('should generate 5 unique codes', function(){
        ...
        codes.forEach(function(code){expect
                          (code.length).toBe(2);expect(codes.indexOf(code)).toBe
                          (codes.lastIndexOf(code)); // check uniqueness })
                      }
                     );
    it('should generate a code consisting of numbers only', 
       function(){ var numbers = "0123456789";
    ...
    it('should generate a code consisting of letters only', function(){
        ...
        code.split('').forEach
            (function(char) 
             {expect(letters).toContain(char);expect(numbers).not.toContain(char);
             }
            )
    });
        it('should generate code with prefix', function()
           {var code = voucher_codes.generate({prefix: "promo-"})[0];expect(code).toMatch
               (/^promo-/);
    });
    ...
    it('should generate code from pattern', function(){
        var code = voucher_codes.generate({
            pattern: "##-###-##"
        })[0];expect(code).toMatch(/^([0-9a-zA-Z]){2}-([0-9a-zA-Z]){3}-([0-9a-zA-Z]){2}$/);
    });
    ...
    it('should detect infeasible config for charset with duplicates', function(){
        var config = {
            count: 2,  charset: "11", length: 2}; // there is only 1 possible code for this config
        expect(function(){voucher_codes.generate(config);
        }
              ).toThrow("Not possible to generate requested number of codes.");
    }
      );
    it('should generate fixed code', function(){
        var config = { count: 1, pattern: "FIXED"};var
            codes = voucher_codes.generate(config);
        expect(codes.length).toEqual(1);expect(codes[0]).toEqual
            ("FIXED");
        ...
        expect((voucher_codes.generate(config, 0)).length).toEqual(1););expect((voucher_codes.generate(config, 0))[0]).toEqual
        ("A000Z");expect((voucher_codes.generate
                          (config, 1))[0]).toEqual("A001Z");expect((voucher_codes.generate
                                                                    (config, 2))[0]).toEqual("A002Z");expect
        ((voucher_codes.generate
          (config, 5))[0]).toEqual("A005Z");expect((voucher_codes.generate(config, 9))[0]).toEqual("A009Z");
        expect((voucher_codes.generate(config, 10))[0]).toEqual
            ("A010Z");
        expect((voucher_codes.generate(config, 11))[0]).toEqual
            ("A011Z");
        expect((voucher_codes.generate(config, 99))[0]).toEqual
            ("A099Z"); expect((voucher_codes.generate(config,100))[0]).toEqua
			l("A100Z");expect((voucher_codes.generate(config, 101))[0]).toEqual("A101Z");
        expect((voucher_codes.generate
				(config, 101))[0]).toEqual("A101Z");expect((voucher_codes.generate)(config, 599))[0]).toEqual("A599Z ) ;expect((voucher_codes.generate(config, 600))[0]).toEqual("A600Z");
    ...
    it('should generate series of sequential codes from numbers charset', function(){
        ...
        const codes = voucher_codes.generate(config, 190); expect(codes.length).toEqual(12);
